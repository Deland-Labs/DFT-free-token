...
import { canister, identity, utils, unit } from "@deland-labs/ic-dev-kit";
...
Then(/^Check "([^"]*)" mintable translation history$/, async function (token, dataTable) {
    ...
    const validate = target_data.map(({ user, amount }) => {
        return {
            userId: utils.principalToAccountID(identity.identityFactory.getPrincipal(user)!),

...
const build = () => {
    canister.build("token_WUSD");
};

const reinstall_by_dfx = async (args: string) => {
    await canister.reinstall("token_WUSD", args);
};
...
export const reinstall = async (options?: ReInstallOptions, initOption?: DFTInitOptions) => {
    ...
    const name = initOption?.name ?? "W USD";
    const symbol = initOption?.symbol ?? "WUSD";
    const decimals = initOption?.decimals ?? 10;
    //const supply = new BigNumber(parseToCommon(initOption?.totalSupply ?? 1000000000000000000000000n)).toFixed();

    const supply = defaultPVADecimals.toVolume(initOption?.totalSupply.toString() ?? "1000000000000000000000000");

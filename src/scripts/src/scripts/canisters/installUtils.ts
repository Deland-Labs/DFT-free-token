...
import {blind_box, exchange} from "~/canisters/names";
...
export const reinstall_with_dev_ids = async (name: string) => {
    let initArgs: InitArgs = {
        dev_named_canister_ids: [
            [{DICP: null}, Principal.fromText(canister.get_id('token_WICP'))],
            [{Registrar: null}, Principal.fromText(canister.get_id('registrar'))],

        ],
    }
    logger.debug(`registrarId ${canister.get_id('registrar')}`)
    logger.debug(`reinstall_with_dev_ids: ${JSON.stringify(initArgs)}`);
